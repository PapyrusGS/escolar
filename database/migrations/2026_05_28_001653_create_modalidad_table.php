...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('modalidad', function (Blueprint $table) {
            $table->id('IdModalidad'); // INT AUTO_INCREMENT PRIMARY KEY
            $table->string('Nombre', 50); // VARCHAR(50) NOT NULL
            $table->integer('DuracionSemanasxMaterias'); // INT NOT NULL
            $table->integer('MaxMaterias'); // INT NOT NULL
            $table->timestamp('FechaRegistro')->useCurrent(); // DATETIME DEFAULT CURRENT_TIMESTAMP
            $table->boolean('Estado')->default(true); // bit DEFAULT TRUE (Se mapea como boolean)
        });
    }
